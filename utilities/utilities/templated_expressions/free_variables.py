...
@singledispatch
def free_variables(self: TemplatedExpression) -> OrderedSet[TemplatedVariable | BaseTemplatedFunction]:
    """Return set of all free variables in the TemplatedExpression.

    This function walks an TemplatedExpression tree, accumulating the set of free variables that it encounters. This function
...
@free_variables.register(TemplatedVariable)
@free_variables.register(BaseTemplatedFunction)
def free_variables_variable(self: TemplatedVariable | BaseTemplatedFunction):
...
def free_variable_names(expression: TemplatedExpression) -> OrderedSet[str]:
    ...
    for variable in free_variables(expression):
        match variable:
            case TemplatedVariable() | BaseTemplatedFunction():
                names.add(variable)
            case _:
                raise ValueError(f"Unexpected variable {variable} in templated expression {expression}")
    return names
